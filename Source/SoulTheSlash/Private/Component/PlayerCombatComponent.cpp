...
void UPlayerCombatComponent::BeginPlay() {
  ...
  if (PlayerAnimInst == nullptr && OwnerCharacter) {
    PlayerAnimInst = Cast<USlashCharacteAnimInstanceBase>(
...
void UPlayerCombatComponent::SetWeapon(ASlashEquippableItemMaster *NewWeapon) {
  ...
  EWeaponStateTypeEnum NewWeaponStateType =
      GetWeaponStateTypeFromWeapon(NewWeaponType);

  if (WeaponSwapBehavior == EWeaponSwapBehavior::WSB_DropToWorld) {
    // Check if we already have a weapon of this state type
    ...
    for (int32 i = 0; i < CurrentAttachedWeaponData.Num(); ++i) {
      if (CurrentAttachedWeaponData[i].WeaponStateType == NewWeaponStateType) {
        ...
        FEquippableStruct *ExistingWeaponStruct = WeaponEquippableSetup.Find(
            CurrentAttachedWeaponData[i].EquippableData.WeaponType);
        if (ExistingWeaponStruct && ExistingWeaponStruct->ActorRef) {
          WeaponToDrop =
              Cast<ASlashEquippableItemMaster>(ExistingWeaponStruct->ActorRef);
          IndexToRemove = i;
        }
        break;
      }
    ...
    if (WeaponToDrop) {
      // FString Msg = FString::Printf(TEXT("Dropping equipped %s to pick up
      // %s"), *WeaponToDrop->GetName(), *NewWeapon->GetName());
...
void UPlayerCombatComponent::DropWeapon(
    ASlashEquippableItemMaster *WeaponToDrop) {
  ...
  WeaponToDrop->InnerSphereColl->SetCollisionEnabled(
      ECollisionEnabled::QueryOnly);
  WeaponToDrop->OuterSphereColl->SetCollisionEnabled(
      ECollisionEnabled::QueryOnly);
  ...
  FEquippableStruct *FoundWeaponItemStruct =
      WeaponEquippableSetup.Find(DroppedWeaponType);
  if (FoundWeaponItemStruct &&
      FoundWeaponItemStruct->ActorRef == WeaponToDrop) {
...
bool UPlayerCombatComponent::HasWeaponOfType(EWeaponTypeEnum WeaponType) const {
  const FEquippableStruct *WeaponStruct =
      WeaponEquippableSetup.Find(WeaponType);
  return WeaponStruct && WeaponStruct->ActorRef &&
         IsValid(WeaponStruct->ActorRef);
}

void UPlayerCombatComponent::AttachEquippableWeapon(
  ...
  if (FoundWeaponItem->ActorRef && IsValid(FoundWeaponItem->ActorRef)) {
    ...
    ASlashEquippableItemMaster *WeaponActor =
        Cast<ASlashEquippableItemMaster>(FoundWeaponItem->ActorRef);
    if (WeaponActor) {
  ...
  if (!FoundWeaponItem->ItemActor) {
    UE_LOG(LogTemp, Warning,
           TEXT("AttachEquippableWeapon: No ItemActor class specified for "
                "ItemType: %d"),
           (int32)WeaponType);
    return;
  }
  ...
  ASlashEquippableItemMaster *SpawnedItem =
      GetWorld()->SpawnActor<ASlashEquippableItemMaster>(
          FoundWeaponItem->ItemActor, SpawnLocation, SpawnRotation,
          SpawnParams);

  if (SpawnedItem) {
    FoundWeaponItem->ActorRef =
        SpawnedItem; // Store the actual actor instance, not the class
    ...
    UE_LOG(LogTemp, Warning,
           TEXT("Failed to spawn equippable item for key: %d"),
           (int32)WeaponType);
  }
}

void UPlayerCombatComponent::AttachItemSocket(
    FEquippableStruct EquippableStruct, AActor *Actor) {
...
void UPlayerCombatComponent::AttachWeaponInternalInfo(
    ASlashEquippableItemMaster *Weapon, EWeaponEquipSource Source) {
  if (!Weapon)
    return;
  ...
  WeaponInstanceData.EquippableData =
      *WeaponEquippableSetup.Find(Weapon->Weapon);
  WeaponInstanceData.EquippableData.ActorRef = Weapon;
...
EWeaponStateTypeEnum UPlayerCombatComponent::GetWeaponStateTypeFromWeapon(
    EWeaponTypeEnum WeaponType) const {
  switch (WeaponType) {
  case EWeaponTypeEnum::Sword1:
...
void UPlayerCombatComponent::EquipWeapon(FEquippableStruct *Item) {
  if (!Item || !Item->ActorRef || !OwnerCharacter)
    return;
  ...
  if (Item) {
    ...
    WeaponItem = Cast<ASlashEquippableItemMaster>(Item->ActorRef);
    if (WeaponItem) {
      // First detach from current socket (back socket)
      // WeaponItem->DetachFromActor(FDetachmentTransformRules::KeepWorldTransform);

      // Then attach to hand socket
      WeaponItem->AttachToComponent(Mesh, AttachmentRules, HandToAttachTo);

      // Update weapon state
      Item->Equipped = true;
      WeaponItem->SetWeaponState(EWeaponState::Equipped);
      WeaponItem->bIsEquipped = true;
      WeaponItem->MainMesh->SetVisibility(true);
      WeaponItem->MainMesh->SetSimulatePhysics(false);
      WeaponItem->MainMesh->SetCollisionEnabled(
          ECollisionEnabled::NoCollision); // First detach from current socket
                                           // (back socket)
    }

    // SetEquipStatus(Item, true);
  }
}
...
void UPlayerCombatComponent::SetEquipStatus(FEquippableStruct *Item,
                                            bool bEquipped) {
  Cast<ASlashEquippableItemMaster>(Item->ActorRef)
      ->MainMesh->SetVisibility(!bEquipped);
  ...
  //
  // 	ASlashEquippableItemMaster* ActorInstance = FoundItem->ActorRef ?
  // Cast<ASlashEquippableItemMaster>(FoundItem->ActorRef) : nullptr; 	if
  // (ActorInstance)
  // 	{
...
UAnimMontage *
UPlayerCombatComponent::GetEquipMontage(FEquippableStruct EquippableStruct) {
...
UAnimMontage *
UPlayerCombatComponent::GetUnequipMontage(FEquippableStruct EquippableStruct) {
